...
class JurisdictionsPostTest < RequestTestCase
  ...
  context "curator API key : post /" do
    ...
    use "incremented jurisdiction count"
      
    test "location header should point to new resource" do
      assert_include "Location", last_response.headers
      new_uri = "http://localhost:4567/jurisdictions/" + parsed_response_body["id"]
    ...
    test "values should be correct in database" do
      jurisdiction = Jurisdiction.find_by_id!(parsed_response_body["id"])
      # TODO: use reload
      @valid_params.each do |key, value|
        assert_equal value, jurisdiction[key]
      end
    end
  end
